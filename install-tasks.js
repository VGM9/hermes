...
function main() {
  ...
  // Merges with existing config so manually-configured fields (autopulse.targets,
  // respawn_mandate, agent_mode) survive re-runs. (hermes#34)
  const windowPattern = basename(workspaceRoot);
  const localConfigPath = join(HERMES_DIR, 'hermes_config.local.jsonc');
  const existing = readExistingLocalConfig(localConfigPath);
  const merged = { ...existing, window_pattern: windowPattern };
  const localConfig = `// Auto-generated by install-tasks.js — do not commit.\n// Overrides hermes_config.jsonc for this workspace.\n// autopulse.targets must be configured manually after first install.\n${JSON.stringify(merged, null, '\t')}\n`;
  ...
  // Create hermes.enable workspace guard marker (hermes#37).
  // The hermes VS Code extension only activates when this file exists, preventing
  // inbox folder creation in every VS Code window. Idempotent — no-op if present.
  const hermesEnablePath = join(workspaceRoot, '_', '.vscode', 'hermes.enable');
  const hermesEnableDir  = join(workspaceRoot, '_', '.vscode');
  if (!existsSync(hermesEnableDir)) mkdirSync(hermesEnableDir, { recursive: true });
  if (!existsSync(hermesEnablePath)) {
    writeFileSync(hermesEnablePath, '', 'utf8');
    console.log(`[hermes:install-tasks] Created workspace guard \u2192 ${hermesEnablePath}`);
  } else {
    console.log(`[hermes:install-tasks] Workspace guard already present: ${hermesEnablePath}`);
  }
}
/** Read hermes_config.local.jsonc, stripping line comments. Returns {} if absent or unparseable. */
function readExistingLocalConfig(p) {
  if (!existsSync(p)) return {};
  try {
    const raw = readFileSync(p, 'utf8').replace(/^\s*\/\/.*$/gm, '');
